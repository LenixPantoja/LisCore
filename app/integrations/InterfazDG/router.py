...
@router.post(
    ...
    description=(
        "Recibe el XML de solicitud de exámenes enviado por el HIS en formato "
        "InterfazDG (DocumentElement / UTF-16). Responde con un XML de acuse "
        "de recibido (MensajeResultado)."
    ),
    response_class=Response,
    status_code=status.HTTP_200_OK,
    responses={
...
async def recibir_solicitud(request: Request) -> Response:
    raw_body = await request.body()
    ...
    acuse = _build_acuse(
        ...
        mensaje=(
            f"Solicitud recibida correctamente. "
            f"Orden: {solicitud.datos_generales.ord_consec if solicitud.datos_generales else 'N/A'}. "
            f"Exámenes: {len(solicitud.examenes)}."
        ),
    )
    return Response(content=acuse, media_type="application/xml", status_code=200)
